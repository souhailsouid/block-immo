...
async function testStatusChange() {
  console.log('🧪 Test du changement de statut...');
  ...
  try {
    ...
    const createResponse = await axios.post(`${API_BASE_URL}/properties`, {
      step: 'basic',
      data: {
        title: 'Test Property - Status Change',
        ...
        yearBuilt: 2020,
        energyClass: 'A',
        description: 'Test property for status change',
        country: 'France',
        state: 'Île-de-France',
        city: 'Paris'
      }
    ...
    // 2. Changer le statut vers COMMERCIALIZED
    console.log('\n2️⃣ Changement de statut vers COMMERCIALIZED...');
    const statusResponse = await axios.put(`${API_BASE_URL}/properties/${propertyId}/status`, {
    ...
    console.log('✅ Changement de statut réussi:', statusResponse.data);
    const newStatus = statusResponse.data.data.newStatus;
    console.log(`🏷️  Nouveau statut: ${newStatus}`);

    // 3. Changer le statut vers FUNDED
    console.log('\n3️⃣ Changement de statut vers FUNDED...');
    const fundedResponse = await axios.put(`${API_BASE_URL}/properties/${propertyId}/status`, {
      status: 'FUNDED'
    }, {
      headers: {
        'Authorization': `Bearer ${TEST_TOKEN}`,
    ...
    console.log('✅ Changement de statut réussi:', fundedResponse.data);
    const finalStatus = fundedResponse.data.data.newStatus;
    ...
    console.log(`📊 Statut intermédiaire: ${newStatus} (devrait être COMMERCIALIZED)`);
    console.log(`📊 Statut final: ${finalStatus} (devrait être FUNDED)`);
    
    if (initialStatus === 'IN_PROGRESS' && newStatus === 'COMMERCIALIZED' && finalStatus === 'FUNDED') {
      console.log('✅ Test réussi ! Le changement de statut fonctionne correctement.');
    } else {
